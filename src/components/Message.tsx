    ...
    content = (
      <Row gutter={[20, 20]}>
        <Col span={24} md={12} xl={8} xxl={6}>
          <Descriptions title="Delivery" column={1} size="small">
            ...
            <Descriptions.Item label="Deliver mode">
              <pre style={{ margin: 0 }}>
                <DeliveryModeTag mode={message.properties.delivery_mode} />
              </pre>
            </Descriptions.Item>
        ...
        {message.properties.headers['x-first-death-exchange'] &&
          ...
          message.properties.headers['x-first-death-reason'] && (
            <Col span={24} md={12} xl={8} xxl={6}>
              <Descriptions title="First death" column={1} size="small">
                <Descriptions.Item label="Exchange">
                  <ExchangeLink
                    name={message.properties.headers['x-first-death-exchange']}
                    vhost={params.vhost}
                  />
                </Descriptions.Item>
                <Descriptions.Item label="Queue">
                  <QueueLink
                    name={message.properties.headers['x-first-death-queue']}
                    vhost={params.vhost}
                  />
                </Descriptions.Item>
          ...
          <Col span={24} md={12} xl={8} xxl={6}>
            <Descriptions title="Death" column={1} size="small">
              ...
              <Descriptions.Item label="Routing keys">
                {death['routing-keys'].map((routingKey) => (
                  <pre style={{ margin: '0 10px 0 0', display: 'inline' }}>
                    {routingKey}
                  </pre>
                ))}
              </Descriptions.Item>
              <Descriptions.Item label="Time">
                <Tooltip title={death.time}>
                  {formatDateTime(death.time * 1000)}
                </Tooltip>
              </Descriptions.Item>
            </Descriptions>
          </Col>
    ...
    <Card
      ...
      tabBarExtraContent={[
        message.payload_encoding && (
          <Tooltip title="Encoding" key="encoding">
            <Tag>{message.payload_encoding}</Tag>
          </Tooltip>
        ),
        message.payload_bytes && (
          <>{formatNumber(message.payload_bytes)} bytes</>
        ),
      ]}
    >
      {content}
